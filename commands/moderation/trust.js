        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addUserOption(option =>
                    option.setName("user").setDescription("User to add as a trusted user.").setRequired(true)
                )
        )
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addUserOption(option =>
                    option.setName("user").setDescription("User to remove from trusted users.").setRequired(true)
                )
        )
        .addSubcommand(subcommand =>
        ...
        try {
            ...
            if (subcommand === "add") {
                ...
                if (moderationData.trustedUsers.has(user.id)) {
                    return interaction.reply({ content: `⚠️ **${user.tag}** is already a trusted user.`, flags: 64 });
                }
                ...
                return interaction.reply({ content: `✅ **${user.tag}** has been added to the trusted list.`, flags: 64 });
            }

            if (subcommand === "remove") {
                const user = interaction.options.getUser("user");
                if (!moderationData.trustedUsers.has(user.id)) {
                    return interaction.reply({ content: `⚠️ **${user.tag}** is not a trusted user.`, flags: 64 });
                }
                ...
                return interaction.reply({ content: `✅ **${user.tag}** has been removed from the trusted list.`, flags: 64 });
            }

            if (subcommand === "list") {
                if (moderationData.trustedUsers.size === 0) {
                    return interaction.reply({ content: "⚠️ No trusted users have been assigned.", flags: 64 });
                }
