...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            const serverData = await getCachedServerData(interaction.client, guild.id);
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: "📌 **Basic Information**", value: `**Name:** ${guild.name}\n**ID:** ${guild.id}\n**Owner:** <@${serverData.ownerId}>\n**Created On:** ${serverData.createdAt}` },
                    { name: "----------------------", value: "⠀" },
                    { name: "👥 **Member Overview**", value: `**Status:**\n🟢 Online: ${serverData.online || 0}\n🟠 Idle: ${serverData.idle || 0}\n🔴 DND: ${serverData.dnd || 0}\n⚪ Offline: ${serverData.offline || 0}`, inline: true },
                    { name: "📱 **Devices**", value: `**Mobile:** ${serverData.mobile || 0}`, inline: true },
                    { name: "📊 **Count**", value: `**Total:** ${serverData.totalMembers || 0}\n🤖 Bots: ${serverData.bots || 0}`, inline: true },
                    { name: "----------------------", value: "⠀" },
                    { name: "💎 **Boosts & Security**", value: `**🔒 Verification:** ${serverData.verificationLevel}\n**💠 Boosts:** ${serverData.boostCount || 0}\n**Level:** ${serverData.boostLevel}` },
                    { name: "----------------------", value: "⠀" },
                    { name: "🛡️ **Server Protection**", value: `📝 **Message Filtering:** ${messageFilterStatus}\n🚨 **Anti-Raid Protection:** ${antiRaidStatus}` },
                    { name: "----------------------", value: "⠀" },
                    { name: "📂 **Assets**", value: `**🔰 Roles & Emojis**\n🎭 Roles: ${serverData.rolesCount || 0}\n😀 Emojis: ${serverData.emojisCount || 0}` }
                )
