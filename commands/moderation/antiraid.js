    ...
    async execute(interaction) {
        try {
            ...
            if (subcommand === "toggle") {
                ...
                return interaction.reply(`✅ **Anti-raid protection has been ${enabled ? "enabled" : "disabled"}.**`);
            }
            ...
            if (subcommand === "settings") {
                ...
                return interaction.reply(`✅ **Anti-raid settings updated:**\nThreshold: ${threshold} joins\nTimeframe: ${timeframe} seconds\nAction: ${action.toUpperCase()}`);
            }
        } catch (error) {
            console.error("Error updating anti-raid settings:", error);
            return interaction.reply({ content: "❌ An error occurred while updating anti-raid settings.", flags: 64 });
        }
    }
};
