    ...
    async execute(interaction) {
        try {
            ...
            if (count < 1 || count > 100) {
                return interaction.reply({ content: "⚠️ You can only delete between **1 and 100** messages at a time.", flags: 64 });
            }
            ...
            if (filteredMessages.size === 0) {
                return interaction.reply({ content: `⚠️ No messages found matching the criteria.`, flags: 64 });
            }

            await channel.bulkDelete(filteredMessages, true).catch(err => {
                console.error(err);
                return interaction.reply({ content: "❌ Failed to delete messages. Some may be older than 14 days.", flags: 64 });
            });

            return interaction.reply({ content: `✅ Successfully deleted **${filteredMessages.size}** messages.`, flags: 64 });
        } catch (error) {
