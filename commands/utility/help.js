    ...
    async execute(interaction) {
        ...
        try {
            ...
            for (const folder of commandFolders) {
                ...
                commands.push(...files.map(file => require(`../${folder}/${file}`).data));
            }
            ...
            const generateEmbed = () => {
                ...
                const commandList = commands.slice(start, end).map(cmd => `**/${cmd.name}** - ${cmd.description || "No description available"}`).join("\n");
            ...
            const buttonRow = new ActionRowBuilder().addComponents(
                ...
                new ButtonBuilder().setCustomId("next").setLabel("▶️").setStyle(ButtonStyle.Primary).setDisabled(page + 1 >= totalPages)
            );

            const message = await interaction.editReply({ embeds: [generateEmbed()], components: [buttonRow], ephemeral: true });
            ...
            collector.on("collect", async btn => {
                if (btn.customId === "prev" && page > 0) page--;
                    ...
                    await btn.update({ content: "⏹️ Help menu closed.", components: [], embeds: [] });
                    collector.stop();
                    return;
                }
            ...
            collector.on("end", async () => {
                try {
                    await interaction.editReply({ content: "⏳ Help menu expired. Run `/help` again.", components: [], embeds: [] });
                } catch (error) {
