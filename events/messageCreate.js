...
module.exports = {
    ...
    async execute(client, message) {
        ...
        if (moderationData.messageFilterEnabled) {
            const FILTER_THRESHOLDS = { normal: 0.75, harsh: 0.60, extreme: 0.45 };
            ...
            try {
                ...
                if (isHateSpeech) {
                    ...
                    message.channel.send(`🚨 <@${userId}>, your message was removed due to inappropriate content.`);
                    
                    const logChannel = message.guild.channels.cache.get(moderationData.logChannelId);
                    if (logChannel) {
                        const embed = new EmbedBuilder()
                            ...
                            .addFields(
                                ...
                                { name: "Filter Level", value: moderationData.filterLevel.toUpperCase() },
        ...
        if (moderationData.levelingEnabled) {
            ...
            if (hasLeveledUp) {
                ...
                message.channel.send(`🎉 <@${userId}> has leveled up to **Level ${userLevel.level}!**`);
            }
        }
        ...
        if (moderationData.rateLimitEnabled) {
            ...
            while (timestamps.length > 0 && now - timestamps[0] > moderationData.rateLimitTimeframe * 1000) {
                timestamps.shift();
            }
            timestamps.push(now);
            ...
            if (timestamps.length >= moderationData.rateLimitThreshold) {
                ...
                try {
                    ...
                    message.channel.send(`⚠️ <@${userId}> has been temporarily restricted due to excessive messaging.`);
                        ...
                        if (logChannel) {
                            const rateLimitEmbed = new EmbedBuilder()
                                ...
                                .addFields(
                                    ...
                                    { name: "Reason", value: "Excessive messaging", inline: false },
                                    { name: "Slowmode Duration", value: `${moderationData.rateLimitDuration} seconds`, inline: true }
                                )
                                .setTimestamp();
