...
module.exports = {
        ...
        .addSubcommand(subcommand =>
            subcommand.setName("remove")
                ...
                .addIntegerOption(option =>
                    option.setName("amount").setDescription("Number of warnings to remove.").setRequired(true)
                )
        )
        .addSubcommand(subcommand =>
            subcommand.setName("reset")
                .setDescription("Reset all warnings.")
        )
        .addSubcommand(subcommand =>
            subcommand.setName("adjust-limit")
                .setDescription("Adjust the warning limit before auto-ban.")
                .addIntegerOption(option =>
                    option.setName("number").setDescription("New warning limit.").setRequired(true)
                )
        )
    ...
    async execute(interaction) {
        try {
            ...
            if (!moderationData) {
                moderationData = await Moderation.findOne({ guildId }) || new Moderation({ guildId });
            ...
            const logChannel = interaction.guild.channels.cache.get(moderationData.logChannelId);

            if (subcommand === "add") {
                const user = interaction.options.getUser("user");
                const reason = interaction.options.getString("reason");

                moderationData.warnings.set(user.id, (moderationData.warnings.get(user.id) || 0) + 1);
                await moderationData.save();
                interaction.client.cache.servers.set(guildId, moderationData);

                const totalWarnings = moderationData.warnings.get(user.id);
                await interaction.reply(`⚠️ **${user.tag}** has been warned. Reason: ${reason} (Total: ${totalWarnings})`);
                ...
                if (logChannel) {
                    const embed = new EmbedBuilder()
                        ...
                        .addFields(
                            ...
                            { name: "Total Warnings", value: `${totalWarnings}/${moderationData.warningLimit}`, inline: true }
                        )
                        .setTimestamp();
                ...
                if (totalWarnings >= moderationData.warningLimit) {
                    await interaction.guild.members.ban(user.id, { reason: "Exceeded warning limit" });
                    ...
                    if (logChannel) {
                        const banEmbed = new EmbedBuilder()
                            ...
                            .addFields(
                                { name: "User", value: `<@${user.id}> (${user.tag})`, inline: true },
                                { name: "Reason", value: "Exceeded warning limit", inline: false }
                            )
                            .setTimestamp();
                ...
                await interaction.reply(`⚠️ **${user.tag}** has **${warnings}** warnings.`);
            }
                ...
                moderationData.warnings.set(user.id, Math.max(0, currentWarnings - amount));
                await moderationData.save();
                interaction.client.cache.servers.set(guildId, moderationData);
                await interaction.reply(`✅ **${amount}** warnings removed from **${user.tag}**. (Total: ${moderationData.warnings.get(user.id)})`);
            }
                ...
                await moderationData.save();
                interaction.client.cache.servers.set(guildId, moderationData);
                await interaction.reply(`✅ The warning limit has been adjusted to **${newLimit}**.`);
            }
        } catch (error) {
