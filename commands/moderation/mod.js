...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (!moderationData) {
                moderationData = await Moderation.findOne({ guildId }) || new Moderation({ guildId, moderators: new Map() });
            ...
            if (subcommand === "add") {
                ...
                if (moderationData.moderators.has(user.id)) {
                    return interaction.reply({ content: `⚠️ **${user.tag}** is already a moderator.`, flags: 64 });
                }

                moderationData.moderators.set(user.id, { addedBy: admin.id, timestamp: Date.now() });
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`**${user.username}** has been granted moderator permissions.`)
                    .addFields(
                ...
                return interaction.reply({ content: `✅ **${user.tag}** has been added as a moderator.`, flags: 64 });
            }
            ...
            if (subcommand === "remove") {
                ...
                if (!moderationData.moderators.has(user.id)) {
                    return interaction.reply({ content: `⚠️ **${user.tag}** is not a moderator.`, flags: 64 });
                }
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`**${user.username}** has been removed from moderator permissions.`)
                    .addFields(
                ...
                return interaction.reply({ content: `✅ **${user.tag}** has been removed as a moderator.`, flags: 64 });
            }

            if (subcommand === "list") {
                if (!moderationData.moderators || moderationData.moderators.size === 0) {
                    return interaction.reply({ content: "⚠️ No moderators have been assigned.", flags: 64 });
                }
            
                const sortedModerators = [...moderationData.moderators.entries()].sort((a, b) => b[1]?.timestamp - a[1]?.timestamp);
            
                const moderatorNames = await Promise.all(sortedModerators.map(async ([id, data]) => {
                    try {
                        const member = await guild.members.fetch(id);
                        const formattedTime = data.timestamp ? `<t:${Math.floor(data.timestamp / 1000)}:R>` : "Unknown Time";
                        const addedBy = data.addedBy ? `<@${data.addedBy}>` : "Unknown User";
            ...
            if (subcommand === "view") {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`**${user.username}** was added as a moderator on ${formattedTime}.`)
                    .addFields({ name: "Added By", value: `<@${data.addedBy}>`, inline: true })
                    .setTimestamp();
