...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (duration > 10080) {
                return interaction.reply({ content: "❌ The timeout duration cannot exceed 7 days (10080 minutes).", flags: 64 });
            }

            const member = await guild.members.fetch(user.id);
            if (!member.moderatable) {
                return interaction.reply({ content: "❌ I cannot timeout this user. They may have a higher role or permissions.", flags: 64 });
            }
            ...
            interaction.reply({ content: `✅ **${user.tag}** has been timed out for **${duration} minutes**.`, flags: 64 });
