...
module.exports = {
    ...
    async execute(client, member) {
        ...
        while (timestamps.length > 0 && now - timestamps[0] > moderationData.antiRaidTimeframe * 1000) {
            timestamps.shift();
        }
        ...
        if (timestamps.length >= moderationData.antiRaidThreshold) {
            ...
            const membersToPunish = timestamps.map(timestamp => member.guild.members.cache.find(m => m.joinedTimestamp === timestamp));
