...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("ban")
        .setDescription("Bans a user from the server and logs it.")
    ...
    async execute(interaction) {
        try {
            ...
            if (!moderationData.logChannelId) {
                return interaction.reply({ content: "⚠️ Please run `/setup` first to create the moderation log channel.", flags: 64 });
            }
            ...
            interaction.reply({ content: `✅ **${user.tag}** has been banned.`, flags: 64 });
